...
function get_wc_booking_data_label( $key, $product ) {
	$labels = apply_filters( 'woocommerce_bookings_data_labels', array(
			'type'     => ( $product->get_resource_label() ? $product->get_resource_label() : __( 'Booking Location', 'woocommerce-bookings' ) ),
...
function wc_bookings_get_summary_list( $booking, $is_admin = false ) {
	...
	$label    = $product && is_callable( array( $product, 'get_resource_label' ) ) && $product->get_resource_label() ? $product->get_resource_label() : __( 'Location', 'woocommerce-bookings' );
